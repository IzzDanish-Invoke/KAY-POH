...
import { bookingAddOns, experiences, packages, tourGuides } from "@/data/data";
  ...
  return <main className="booking-page">
    ...
    <div className="booking-layout">
      <section className="booking-content">
        <div className="booking-intro"><p className="booking-kicker">Build your Ipoh trip</p><h1>What brings you<br /><em>to Ipoh?</em></h1><p>Choose a complete getaway or a shorter local experience. You can personalise everything before confirming.</p></div>

        <section className="booking-step"><header><span>01</span><div><h2>Choose your trip type</h2><p>You can book accommodation and activities together, or just join us for a few hours.</p></div></header><div className="type-options"><button className={type === "package" ? "active" : ""} onClick={() => switchType("package")}><span>▣</span><div><strong>Packages</strong><small>Complete 2D1N and 3D2N getaways</small></div><i>{type === "package" ? "●" : "○"}</i></button><button className={type === "experience" ? "active" : ""} onClick={() => switchType("experience")}><span>◎</span><div><strong>Experiences</strong><small>Short tours from 3 hours to half a day</small></div><i>{type === "experience" ? "●" : "○"}</i></button></div></section>
        ...
        <section className="booking-step"><header><span>04</span><div><h2>Choose your local guide</h2><p>Your request goes directly to the guide. They’ll confirm their availability before payment.</p></div></header><div className="booking-guide-grid">{tourGuides.map((item) => <button className={guideId === item.id ? "active" : ""} onClick={() => setGuideId(item.id)} key={item.id}><span className={`booking-avatar avatar-${item.color}`}>{item.initials}</span><div><strong>{item.name}</strong><small>{item.title}</small><p>★ {item.rating} <i>({item.reviews})</i> · {item.languages.slice(0,2).join(" / ")}</p></div><b>{guideId === item.id ? "Selected ✓" : "Choose"}</b></button>)}</div></section>

        <section className="booking-step"><header><span>05</span><div><h2>Add a little extra</h2><p>Optional touches to make the trip work better for you.</p></div></header><div className="addon-list">{availableAddOns.map((item) => <label key={item.id}><input type="checkbox" checked={addOns.includes(item.id)} onChange={() => toggleAddOn(item.id)} /><span /><div><strong>{item.name}</strong><small>{item.description}</small></div><b>+RM{item.price}</b></label>)}</div></section>
      </section>

      <aside className="booking-summary"><div className="summary-card"><p className="booking-kicker">Your booking</p><div className={`summary-art ${"color" in selected ? selected.color : ""}`}><span>{duration}</span><small>{type === "package" ? "CURATED GETAWAY" : "LOCAL EXPERIENCE"}</small></div><h2>{selected.title}</h2><div className="summary-guide"><span className={`booking-avatar avatar-${guide.color}`}>{guide.initials}</span><div><small>Your preferred guide</small><strong>{guide.name}</strong></div><span>★ {guide.rating}</span></div><dl><div><dt>Date</dt><dd>{date || "Choose a date"}</dd></div><div><dt>Travellers</dt><dd>{guests}</dd></div><div><dt>Base price</dt><dd>{selected.currency}{selected.price} × {guests}</dd></div>{addOns.length > 0 && <div><dt>Add-ons</dt><dd>RM{addOnTotal}</dd></div>}</dl><div className="summary-total"><span>Total estimate<small>Taxes included</small></span><strong>RM{total.toLocaleString("en-MY")}</strong></div><button className="booking-primary" disabled={!date} onClick={() => setSubmitted(true)}>Request this booking <span>→</span></button>{!date && <p className="date-reminder">Choose a preferred date to continue.</p>}<p className="booking-assurance">No payment yet. Your guide confirms first.</p></div><div className="summary-help"><span>?</span><div><strong>Not sure what to choose?</strong><p>Talk to a local trip planner.</p></div><a href="mailto:hello@kaypoh.tours">Get help →</a></div></aside>
    </div>
  </main>;
}
