...
export default function BookingFlow() {
  ...
  const travellerRates = "days" in selected ? selected.rates.filter((rate) => !rate.label.startsWith("Caregiver")) : [];
  const selectedRate = travellerRates.find((rate) => rate.label === rateLabel) ?? travellerRates[0];
  const unitPrice = selectedRate?.price ?? selected.price;
